                        ...
                        <div class="col-lg-6 p-2 mt-2   card">
                            ...
                            <?php
                                ...
                                if (isset($_POST['add-to-wishlist'])) {
                                    ...
                                    $stid = oci_parse($connection, "SELECT fk1_user_id FROM wishlist, wishlist_product
                            WHERE wishlist.wishlist_id = wishlist_product.wishlist_id and wishlist.fk1_user_id = '$user_id' and wishlist_product.product_id = '$pid'");
                                    oci_execute($stid);

                                    if ($row = oci_fetch_array($stid, OCI_ASSOC)) {
                                        echo "You already have this item in your <a href = './wishlist.php'>Wishlist</a>";
                                    } else {



                                        $stid = oci_parse($connection, "INSERT INTO WISHLIST_PRODUCT (wishlist_id, product_id)
                                        VALUES ((SELECT wishlist_id FROM wishlist WHERE fk1_user_id = '$user_id'),
                                        '$pid')");
                                        if (oci_execute($stid)) echo "Item saved to wishlist";
                                    }
                                }
                        ...
                        <div class="col-lg-6 reviews">
                            ...
                            <div class="border p-2 my-3 " style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                                <div class="py-2 ">
                                    ...
                                    ?>
                                                <form action="./product.php?pid=<?php echo $pid; ?>" method="post">
                                                    <input type="number" name="reviewid" id="reviewid" hidden value="<?php echo $review_id ?>">
                                ...
                                <i class="fas fa-star-half-alt"></i>
                                        ...
                                        if (isset($_POST['delete'])) {
                                            $reviewidtodelete = $_POST['reviewid'];
                                            $stidd = oci_parse($connection, "DELETE FROM review WHERE review_id = '$reviewidtodelete'");
                                            oci_execute($stidd);
                                            $_POST['delete'] = null;
                                        ?>
                                            <script>
                                                location.replace('product.php?pid=<?php echo $pid; ?>');
                                            </script>
                                    <?php
                                        }

                                        echo '<div class="review-title">' . $review_title . "</div>";
                                        echo $review . "<hr><br><br>";
                                        $number_of_reviews++;
                                    }
                                    if ($number_of_reviews < 1) echo '<lead>No review yet. Be the first!</lead>'
